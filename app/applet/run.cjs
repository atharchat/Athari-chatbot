...
$('.PageText').each(function(index) {
    ...
    if (!isSkipping) {
        ...
        if (text.length > 0) finalBlocks.push(text);
    }
});
const result = finalBlocks.join('\n\n');
console.log('باب القول بالمذهب: ' + result.includes('باب القول بالمذهب'));
console.log('من زعم أن الإيمان هو القول: ' + result.includes('من زعم أن الإيمان هو القول'));
console.log('ومن لم ير الاستثناء في الإيمان: ' + result.includes('ومن لم ير الاستثناء في الإيمان'));
console.log('والولاية بدعة، والبراءة بدعة؛ وهم الذين : ' + result.includes('والولاية بدعة، والبراءة بدعة؛ وهم الذين'));
