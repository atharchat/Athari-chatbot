...
async function main() {
    ...
    for (let i = 0; i < deletedFiles.length; i++) {
        if (deletedFiles[i].includes('books/')) {
            await deleteBookVectors(deletedFiles[i]);
        }
    }

    for (let i = 0; i < filesToProcess.length; i++) {
        if (filesToProcess[i] && filesToProcess[i].includes('books/') && (filesToProcess[i].endsWith('.txt') || filesToProcess[i].endsWith('.htm') || filesToProcess[i].endsWith('.html'))) {
            await processBook(filesToProcess[i]);
        }
    }
}
