            ...
            try {
                ...
                if (data.error) {
                    if (data.error.code === 429) {
                        console.log('تم حظر المعدل (429 Rate Limit)، ننتظر 10 ثواني قبل إعادة المحاولة...');
                        await delay(10000);
                        retries++;
                        continue;
                    }
...
async function processBook(filePath) {
    const parts = filePath.split('/');
    const topicFolder = parts.length > 2 && parts[0] === 'test_sync' ? parts[2].toLowerCase() : (parts.length > 1 ? parts[1].toLowerCase() : 'aqeedah');
    const fileName = parts[parts.length - 1];
    const sourceName = fileName.replace(/\.[^/.]+$/, "");
    ...
    for (let i = 0; i < chunks.length; i += 50) {
        ...
        try {
            ...
            console.log(`تم رفع المتجهات الدفعة ${i / 50 + 1} لكتاب ${sourceName}`);
