...
async function processBook(filePath) {
    ...
    for (let i = 0; i < chunks.length; i += 50) {
        ...
        try {
            ...
            await index.upsert(vectors); // Or { records: vectors } depending on Pinecone version
...
async function main() {
    for (const file of filesToDelete) {
        // Only checking endsWith instead of includes to avoid test files if not intended, but keeping includes for now
